...
//! `/api/prices`, `/api/catalog`, `/api/fx`). A `vercel.json` rewrite sends each of those paths
...
use cryptoally_api::infrastructure::postgres::{
    pool, PgApiKeyRepository, PgAssetRepository, PgCatalogRepository, PgFxRepository, PgPriceRepository,
};
...
async fn main() -> Result<(), Error> {
    ...
    let fx_repo = Arc::new(PgFxRepository::new(pool));
